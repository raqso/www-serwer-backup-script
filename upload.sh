...
# Method to delete files from google drive. Requires 2 arguments file path id and access token.
function deleteFile(){
	if [ -n "$FILE_ID_TO_DELETE" ]
	then
		ACCESS_TOKEN="$1"

		# Curl command to delete the file from google drive.
		log "Deleting file $FILE_ID_TO_DELETE from google drive..."
		curl \
		-X DELETE \
		-H "Authorization: Bearer ${ACCESS_TOKEN}" \
		"https://www.googleapis.com/drive/v2/files/{$FILE_ID_TO_DELETE}" \
			$curl_args
	fi
}
...
function findId(){
	...
	# Curl command to delete the file from google drive.
	log "Searching oldest backup file $TO_REMOVE in google drive..."
	RESP=`/usr/bin/curl \
	-X GET \
	-H "Authorization: Bearer ${ACCESS_TOKEN}" \
	"https://www.googleapis.com/drive/v2/files?q=title%20%3D%20'$TO_REMOVE.gz'" \
		$curl_args`

	fileId=`echo "$RESP" | jsonValue "id" 1`

	FILE_ID_TO_DELETE="$fileId"
	log "Id for $TO_REMOVE is: $FILE_ID_TO_DELETE"
}
